...
void	*make_data(t_cub3d *game, char *file, t_dep s)
{
	...
	if (fd < 0)
		ft_close(1);
	close(fd);
	data = mlx_xpm_file_to_image(game->mlx, file, &s.x, &s.y);
	if (!data)
	{
		if (!file)
			ft_putstr_fd("NULL", 1);
...
void	make_data_map(t_cub3d *game)
{
	...
	{
		s = 0;
		while (41 > game->textures[i].addr[s] || game->textures[i].addr[s] > 126)
			s ++;
		file = ft_strdup(game->textures[i].addr + s);
		s = ft_strlen(file) + 1;
		while(file[--s] < '!' || file[--s] > '~')
			file[s] = '\0';
		game->textures[i].img = make_data(game, file, size);
		free(file);
	}
}
